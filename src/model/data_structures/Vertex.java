...
public class Vertex<K extends Comparable<K>, V extends Comparable<V>> implements Comparable<Vertex<K, V>> {
	...
	public void bfs() {
		...
		initializeBFS(cola);
		processBFSQueue(cola);
	}

	private void initializeBFS(ColaEncadenada<Vertex<K, V>> cola) {
		mark();
		cola.enqueue(this);
	}

	private void processBFSQueue(ColaEncadenada<Vertex<K, V>> cola) {
		while (cola.peek() != null) {
			Vertex<K, V> actual = cola.dequeue();
			exploreEdges(actual, cola);
		}
	}

	private void exploreEdges(Vertex<K, V> actual, ColaEncadenada<Vertex<K, V>> cola) {
	...
	public void topologicalOrder(ColaEncadenada<Vertex<K, V>> pre, ColaEncadenada<Vertex<K, V>> post,
	...
	public ILista<Edge<K, V>> mstPrimLazy() {
		...
		initializePrim(cola, this); // Inicialización de Prim
		processPrimEdges(cola, mst); // Procesamiento de los bordes

		return mst;
	}

	// Método para inicializar el algoritmo de Prim con los bordes iniciales
	private void initializePrim(MinPQ<Float, Edge<K, V>> cola, Vertex<K, V> startVertex) {
		addEdgesToMinPQ(cola, startVertex);
	}

	// Método para procesar los bordes en Prim
	private void processPrimEdges(MinPQ<Float, Edge<K, V>> cola, ILista<Edge<K, V>> mst) {
		while (!cola.isEmpty()) {
			...
			if (!dest.marked) {
				try {
					...
					addEdgesToMinPQ(cola, dest); // Añadir los bordes del vértice destino
				} catch (PosException | NullException e) {
					e.printStackTrace();
				}
			}
	...
	public ITablaSimbolos<K, NodoTS<Float, Edge<K, V>>> minPathTree() {
		ITablaSimbolos<K, NodoTS<Float, Edge<K, V>>> tablaResultado = new TablaHashLinearProbing<K, NodoTS<Float, Edge<K, V>>>(
				2);
	...
	public void relaxDijkstra(ITablaSimbolos<K, NodoTS<Float, Edge<K, V>>> tablaResultado,
			MinPQIndexada<Float, K, Edge<K, V>> colaIndexada, Vertex<K, V> actual, float pesoAcumulado) {
		actual.mark();
		processEdgesDijkstra(tablaResultado, colaIndexada, actual, pesoAcumulado);
	}

	private void processEdgesDijkstra(ITablaSimbolos<K, NodoTS<Float, Edge<K, V>>> tablaResultado,
			MinPQIndexada<Float, K, Edge<K, V>> colaIndexada, Vertex<K, V> actual, float pesoAcumulado) {
		for (int i = 1; i <= actual.edges().size(); i++) {
			Edge<K, V> arcoActual = null;
			try {
				arcoActual = actual.edges().getElement(i);
				updateDijkstraNode(tablaResultado, colaIndexada, actual, arcoActual, pesoAcumulado);
			} catch (PosException | VacioException e) {
				e.printStackTrace();
			}
		}
	...
	private void updateDijkstraNode(ITablaSimbolos<K, NodoTS<Float, Edge<K, V>>> tablaResultado,
			MinPQIndexada<Float, K, Edge<K, V>> colaIndexada, Vertex<K, V> actual, Edge<K, V> arcoActual,
			float pesoAcumulado) {

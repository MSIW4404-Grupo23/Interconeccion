...
public class GrafoListaAdyacencia<K extends Comparable<K>, V extends Comparable<V>> {
	...
	public void bfs(K id) throws PosException {
		Vertex<K, V> inicio = getVertex(id);
		performBFS(inicio);
		unmark();
	}

	private void performDFS(Vertex<K, V> inicio) {
		inicio.dfs(null);
	}

	private void performBFS(Vertex<K, V> inicio) {
		inicio.bfs();
	}

	public Edge<K, V> arcoMin() {
		return calculateExtremeEdge(true);
	}

	public Edge<K, V> arcoMax() {
		return calculateExtremeEdge(false);
	}

	private Edge<K, V> calculateExtremeEdge(boolean isMin) {
		Edge<K, V> extreme = null;
		try {
			extreme = arcos.getElement(1);
			float extremeWeight = arcos.getElement(1).getWeight();
			for (int i = 2; i <= arcos.size(); i++) {
				float weight = arcos.getElement(i).getWeight();
				if ((isMin && weight < extremeWeight) || (!isMin && weight > extremeWeight)) {
					extreme = arcos.getElement(i);
